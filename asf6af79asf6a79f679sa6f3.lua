...
local WorkZFolder = "WorkZ"
local ConfigurationFolder = WorkZFolder.."/Configurations"
local ConfigurationExtension = ".wrkz"
...
local WorkZLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across WorkZ
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across WorkZ
...
local WorkZ = game:GetObjects("rbxassetid://17859137185")[1]
...
elseif syn.protect_gui then 
	syn.protect_gui(WorkZ)
	WorkZ.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	WorkZ.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	WorkZ.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == WorkZ.Name and Interface ~= WorkZ then
			Interface.Enabled = false
			Interface.Name = "WorkZ-Old"
		end
	end
else
...
local Main = WorkZ.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

WorkZ.DisplayOrder = 100
...
local Notifications = WorkZ.Notifications

local SelectedTheme = WorkZLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = WorkZLibrary.Theme[ThemeName]
	for _, obj in ipairs(WorkZ:GetDescendants()) do
	...
	WorkZ.Main.BackgroundColor3 = SelectedTheme.Background
	WorkZ.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	WorkZ.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	WorkZ.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	WorkZ.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	WorkZ.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	WorkZ.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if WorkZLibrary.Flags[FlagName] then
			spawn(function() 
				if WorkZLibrary.Flags[FlagName].Type == "ColorPicker" then
					WorkZLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if WorkZLibrary.Flags[FlagName].CurrentValue or WorkZLibrary.Flags[FlagName].CurrentKeybind or WorkZLibrary.Flags[FlagName].CurrentOption or WorkZLibrary.Flags[FlagName].Color ~= FlagValue then WorkZLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			WorkZLibrary:Notify({Title = "Flag Error", Content = "WorkZ was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(WorkZLibrary.Flags) do
...
function WorkZLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= WorkZLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("WorkZ | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	WorkZLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function WorkZLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "WorkZ Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by 53_work"
	if Settings.LoadingTitle ~= "WorkZ Interface Suite" then
		LoadingFrame.Version.Text = "WorkZ UI"
	end
	...
	pcall(function()
		...
		if not isfolder(WorkZFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(WorkZFolder.."/Discord Invites") then
			makefolder(WorkZFolder.."/Discord Invites")
		end
		if not isfile(WorkZFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(WorkZFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"WorkZ RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(WorkZFolder.."/Key System") then
			makefolder(WorkZFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("WorkZ | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(WorkZFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(WorkZFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(WorkZFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						WorkZLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("WorkZ | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					WorkZLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("WorkZ | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("WorkZ | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					WorkZLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("WorkZ | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					WorkZLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("WorkZ | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					WorkZLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("WorkZ | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					WorkZLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function WorkZLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				WorkZLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, WorkZLibrary.LoadConfiguration, WorkZLibrary)
